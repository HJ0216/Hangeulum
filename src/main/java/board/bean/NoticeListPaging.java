...
public class NoticeListPaging {
	...
	public void makePagingHTML() {
		...
		if (startP != 1) { // [이전] 블럭을 출력하는 경우
			pagingHTML.append("<button type='button' class='contentPagingBefore' onclick='noticeListPaging("
					+ (startP - 1) + ")'>이전</button>");
		...
		for (int i = startP; i <= endP; i++) {
			if (i == currentPage) {
				pagingHTML.append(
						"<button class='contentPagingNumber' id='contentPagingCurrnet' onclick='noticeListPaging(" + i
								+ ")'>" + i + "</button>");
			} else {
				pagingHTML.append(
						"<button class='contentPagingNumber' onclick='noticeListPaging(" + i + ")'>" + i + "</button>");
			}
		}

		if (endP < totalP) { // [다음] 블럭을 출력하는 경우
			pagingHTML.append("<button type='button' class='contentPagingAfter' onclick='noticeListPaging(" + (endP + 1)
					+ ")'>다음</button>");
		}
